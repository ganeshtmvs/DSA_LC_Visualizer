...
bool compare_impl(int idx1, int idx2, int line = 0) {
    stepCount++;
    check_step_limit();
    int val1 = (idx1 >= 0 && idx1 < (int)global_nums.size()) ? global_nums[idx1] : idx1;
    int val2 = (idx2 >= 0 && idx2 < (int)global_nums.size()) ? global_nums[idx2] : idx2;
...
int main() {
    ...
    vector<string> tree_nodes = {"1", "3", "5", "7", "9"};
